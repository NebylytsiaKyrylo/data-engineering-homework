...
def test_process_sales_data_success(
    mock_logger_info, mock_write_avro_file, mock_read_json_file, 
    mock_os_listdir, mock_os_makedirs, mock_os_path_isdir
):
    ...
    file1_data = [
        {"client": "Client 1", "purchase_date": "2024-05-07", "product": "Product A", "price": 100}
    ]
    file2_data = [
        {"client": "Client 2", "purchase_date": "2024-05-07", "product": "Product B", "price": 200}
    ]
    ...
    assert mock_logger_info.call_count >= 7  # At least 7 log messages should be recorded
...
def test_process_sales_data_read_json_file_error(
    mock_logger_error, mock_logger_info, mock_read_json_file, 
    mock_os_listdir, mock_os_makedirs, mock_os_path_isdir
):
    ...
    mock_logger_error.assert_has_calls([
        mock.call(f"Error reading file {test_filepath}: {error_msg}", exc_info=True),
...
def test_process_sales_data_write_avro_file_error(
    mock_logger_error, mock_logger_info, mock_write_avro_file, 
    mock_read_json_file, mock_os_listdir, mock_os_makedirs, mock_os_path_isdir
):
    ...
    # Assert error logging was called with both messages
    mock_logger_error.assert_has_calls([
        mock.call(f"Error saving file {test_output_filepath}: {error_msg}", exc_info=True),
        mock.call(f"An unexpected error occurred: {error_msg}", exc_info=True)
    ])
...
def test_process_sales_data_unexpected_error(
    mock_logger_exception, mock_logger_info, mock_os_listdir, 
    mock_os_makedirs, mock_os_path_isdir
):
