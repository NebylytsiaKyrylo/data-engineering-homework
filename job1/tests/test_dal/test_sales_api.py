...
from job1.dal.sales_api import (
    get_sales_per_page,
    API_URL,
    ERR_TOKEN_MISSING,
    MAX_RETRIES,
...
@mock.patch("job1.dal.sales_api.AUTH_TOKEN", "test_token")  # Mock environment
# variable
@mock.patch("job1.dal.sales_api.requests.get")  # Mock requests.get call
...
@mock.patch("job1.dal.sales_api.AUTH_TOKEN", "test_token")
...
def test_get_sales_per_page_unauthorized_401(mock_requests_get):
    ...
    assert "Non-retryable HTTP error" in str(excinfo.value)
    assert "fetching page 1" in str(excinfo.value)
...
@mock.patch("job1.dal.sales_api.AUTH_TOKEN", None)
...
@mock.patch("job1.dal.sales_api.AUTH_TOKEN", "test_token")
@mock.patch("job1.dal.sales_api.requests.get")
@mock.patch("job1.dal.sales_api.time.sleep")  # Mock sleep to avoid waiting
