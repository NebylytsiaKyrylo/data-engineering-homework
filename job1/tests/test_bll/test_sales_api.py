...
@mock.patch("job1.bll.sales_api.local_disk.prepare_storage_dir")
@mock.patch("job1.bll.sales_api.sales_api.get_sales_per_page")
@mock.patch("job1.bll.sales_api.local_disk.save_page_to_disk")
@mock.patch("job1.bll.sales_api.logger.info")
def test_save_sales_to_local_disk_success_multiple_pages(
    mock_logger_info,
    mock_save_page_to_disk,
    mock_get_sales_per_page,
    mock_prepare_storage_dir,
):
    ...
    mock_save_page_to_disk.assert_has_calls(
        [
            mock.call(
                page1_data, dir_path=test_dir, filename=f"sales_{test_date}_1.json"
            ),
            mock.call(
                page2_data, dir_path=test_dir, filename=f"sales_{test_date}_2.json"
            ),
        ]
    )
    ...
    assert (
        mock_logger_info.call_count >= 8
    )  # At least 8 log messages should be recorded
...
@mock.patch("job1.bll.sales_api.local_disk.save_page_to_disk")
@mock.patch("job1.bll.sales_api.logger.info")
@mock.patch("job1.bll.sales_api.logger.error")
def test_save_sales_to_local_disk_prepare_storage_dir_error(
    ...
    mock_logger_error.assert_called_once_with(
        f"File system error while saving data to {test_dir}: {error_msg}"
    )
...
def test_save_sales_to_local_disk_get_sales_per_page_error(
    ...
    # Assert error logging was called
    mock_logger_error.assert_called_once_with(
        f"Failed to connect to the sales API: {error_msg}"
    )
...
@mock.patch("job1.bll.sales_api.local_disk.save_page_to_disk")
@mock.patch("job1.bll.sales_api.logger.info")
@mock.patch("job1.bll.sales_api.logger.exception")
